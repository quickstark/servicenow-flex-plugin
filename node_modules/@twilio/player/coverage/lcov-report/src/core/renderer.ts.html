...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { WaveSurferParams } from "wavesurfer.js"
...
&nbsp;
  public prepareForDraw(
    ...
    // parameter or if normalize=true from the largest value in the peak set
    let absmax = 1 / this.params.barHeight
    if (this.params.normalize) {
      const allValues = hasMultipleChannels
        ? (<span class="branch-0 cbranch-no" title="branch not covered" >peaks as number[][]).reduce(<span class="fstat-no" title="function not covered" >(a</span>cc, peaks) =&gt; <span class="cstat-no" title="statement not covered" >acc.concat(peaks),</span> [])</span>
        : (peaks as number[])
      const max = util.max(allValues)
      const min = util.min(allValues)
      absmax = -min &gt; max ? <span class="branch-0 cbranch-no" title="branch not covered" >-min </span>: max
    }
    const height = this.params.height * this.params.pixelRatio
    const halfH = height / 2
&nbsp;
    const length = hasMultipleChannels
      ? (<span class="branch-0 cbranch-no" title="branch not covered" >peaks[0] as number[]).length</span>
      : peaks.length
...
&nbsp;
      let heights: number[]
      <span class="missing-if-branch" title="if path not taken" >I</span>if (hasMultipleChannels) {
        const peakAverageValues = (<span class="cstat-no" title="statement not covered" >peaks as number[][]).map(<span class="fstat-no" title="function not covered" >p </span>=&gt;</span>
<span class="cstat-no" title="statement not covered" >          this.getPeakAverageValue(p, peakIndex, step)</span>
        )
<span class="cstat-no" title="statement not covered" >        heights = peakAverageValues.map(<span class="fstat-no" title="function not covered" >v </span>=&gt; <span class="cstat-no" title="statement not covered" >Math.round((v / absmax) * halfH))</span></span>
      } else {
        const peakAverageValue = this.getPeakAverageValue(
          peaks as number[],
          peakIndex,
          step
        )
...
&nbsp;
      ...
      channelsSortedByBarHeight = channelsSortedByBarHeight.sort(
<span class="fstat-no" title="function not covered" >        (a</span>, b) =&gt; <span class="cstat-no" title="statement not covered" >heights[b] - heights[a]</span>
      )
&nbsp;
...
&nbsp;
   ...
   */
  public getPeakAverageValue(peaks: number[], index: number, radius: number) {
    if (index &lt; 0) {
      return 0
    }
    let sum = 0
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Tue Oct 06 2020 21:15:23 GMT+0200 (Central European Summer Time)
            </div>
        </div>
